...
class BadgeFormatter:
    ...
    def format_code_badges(cls, entry):
        ...
        languages = (
            entry["language"]
            if isinstance(entry.get("language"), list)
            else [entry.get("language", "Code")]
        )
        ...
        for i, code_url in enumerate(codes):
            ...
            badges.append(
                cls.create_badge(lang_combined, lang_combined, color, code_url)
            )
            ...
            for item in data:
                if isinstance(item, dict):
                    ...
                    if url:
                        ...
                        badges.append(
                            cls.create_badge(badge_type, badge_type, color, url)
                        )
